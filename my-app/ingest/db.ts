...
function buildUpsertQuery(rows: ParkingTicketRow[]): {
  ...
  const text = `
    ...
    ON CONFLICT (summons_number) DO UPDATE SET
      ...
      house_number = COALESCE(EXCLUDED.house_number, parking_ticket.house_number),
      county = EXCLUDED.county,
      precinct = EXCLUDED.precinct,
      street_name = COALESCE(EXCLUDED.street_name, parking_ticket.street_name),
      intersecting_street = COALESCE(EXCLUDED.intersecting_street, parking_ticket.intersecting_street),
